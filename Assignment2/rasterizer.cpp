...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    // Find the maximum and minimum values of three vertex coordinates
    ...
    // Make the range with int value to prepare for iteration
    ...
    for (int x = x_min; x <= x_max; x++)
    {
        for (int y = y_min; y <= y_max; y++)
        {
            ...
            {
                // Construct Barycentric Coordinates and get the values for alpha, beta, gamma
                auto tup = computeBarycentric2D(x, y, t.v);

                // Input values for alpha, beta, and gamma
                float alpha;
                float beta;
                float gamma;
